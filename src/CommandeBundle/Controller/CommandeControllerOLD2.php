...
class CommandeController extends Controller
{
    ...
    */
    ...
    public function verificationReservation($article, $commande, $quantiteDemandee){
        ...
        if($compteurReservation > $article->getQuantite() || $quantiteDemandee > $article->getQuantite() || $quantiteDemandee+$compteurReservation > $article->getQuantite()){
            
            $this->addFlash('alert', "L'article '".$article->getLibelle()."' n'a pas rajouté à la commande '".$commande->getLibelle()."' car pour la date ".$commande->getDateEvenement()->format('d/m/Y')." il y en à ". $compteurReservation." de reservés, vous en demandez ".$quantiteDemandee." et en stock il y en a ".$article->getQuantite());
    ...
    public function suppressionArticleAction(Request $request){
        ...
        //@todo remettre les articles dans le stock SEULEMENT SI ACTION = VENTE BORDEL
        if($commandeArticle->getAction() == "Vente"){
